...
class Robot
{
    ...
    {
        $params = ["category" => 2, "mode" => ($eco ? 1 : 2), "modifier" => 2];
    ...
    {
        $params = ["category" => 2, "mode" => 1, "modifier" => 2];
    ...
    {
        $result = ["message" => "no serial or secret"];

        if ($this->serial !== false && $this->secret !== false) {
            $payload = ["reqId" => "1", "cmd" => $command];
            ...
            $result = Api::request($this->baseUrl . $this->serial . "/messages", $payload, "POST", $headers);
        }
